      ...
      <main class="my-8">
        <div class="container h-full mx-auto px-6">
            ...
            <div class="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mt-6">
                <div class="w-full max-w-sm mx-auto rounded-md shadow-md overflow-hidden" v-for="(categori) in getCategori.data.slice(0,12)" :key="categori.id">
                    ...
                    <div class="px-5 py-3">
                        <h3 class="text-black uppercase">{{ categori.name }}</h3>
                        <span class="text-black mt-2">{{ categori.slug }}</span>
                    </div>
                </div>
                </div>
        </div>
